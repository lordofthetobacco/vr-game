...
float geometrySmith(vec3 N,vec3 V,vec3 L,float roughness){
    float ggxV=geometrySchlickGGX(max(dot(N,V),0.),roughness);
    float ggxL=geometrySchlickGGX(max(dot(N,L),0.),roughness);
    return ggxV*ggxL;
}
...
void main(){
    ...
    float roughness=clamp(texture(roughnessTex,fragUV).g,.04,1.);
    ...
    vec3 nTex=texture(normalTex,fragUV).xyz*2.-1.;
    mat3 TBN=mat3(T,B,N);
